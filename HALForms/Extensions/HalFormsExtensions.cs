...
public static class HalFormsExtensions
{
    ...
    public static HalBuilderBase<HalFormsDocument> WithTemplateFromModelAndErrors<T>(
        this HalBuilderBase<HalFormsDocument> builder,
        string templateName,
        ...
        string? title = null)
    {
        if (builder is not HalFormsBuilder formsBuilder)
            throw new ArgumentException("Builder must be a HalFormsBuilder");

        formsBuilder.WithTemplateFromModel(templateName, method, target, modelInstance, title);
        var errors = modelState.ToHalFormErrors();
        return formsBuilder.WithErrors(templateName, errors);
    }
    ...
    public static HalBuilderBase<HalFormsDocument> WithTemplateFromModel<T>(
        this HalBuilderBase<HalFormsDocument> builder,
        string templateName,
        HalFormHttpMethod method,
        string target,
    ...
    {
        if (builder is not HalFormsBuilder formsBuilder)
            throw new ArgumentException("Builder must be a HalFormsBuilder", nameof(builder));

        return formsBuilder.WithTemplate(templateName, templateBuilder =>
        {
            templateBuilder
                .WithMethod(method)
                .WithTarget(target);
